...
#Muestre en pantalla y retorne los jugadores que tengan un imc menor a 18 e imprima que debe presentarse a consulta medica
...
def verificarJugadores(jugadores, imcJugadores):
    ...
    for i in range(len(jugadores)):
        if imcJugadores[i] < 18:
            lista_menores.append(jugadores[i])
...
#los jugadores y devolverlos en una lista
...
def maxJugadores(jugadores, np_imc):
    ...
    return jugadores_imcAlto
    #print(f'El indice de IMC mas alto es/son los jugadores: {", ".join(jugadores_imcAlto)} con un ICM: {imc_max}')

#Determine una funcion calcularPromedio que retorne el promedio de la lista de IMC de los jugadores.
